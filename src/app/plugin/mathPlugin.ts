...
export function isMathQuery(query: string): boolean {
	...
	const hasMathExpression =
		/[\d\s.+\-*/()^!%=]+/.test(query) && /[+\-*/^!=]/.test(query);

	// Check for equals sign or question mark with numbers
	const hasEqualsOrQuestion = /[\d\s.+\-*/()]+\s*[=?]\s*$/.test(query);

	return hasKeywords || hasMathExpression || hasEqualsOrQuestion;
}
